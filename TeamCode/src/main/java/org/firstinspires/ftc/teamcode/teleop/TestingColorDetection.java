...
public class TestingColorDetection extends LinearOpMode
{
    ...
    public class Pipeline extends OpenCvPipeline {
        ...
        List<Integer> ELEMENT_COLOR = Arrays.asList(240, 100, 0); //(Hue, Sateration, Value), Set to blue alliance at first
...
//            input.submat(rightRect).setTo(avgColorRight);
        ...
        public double color_distance(Scalar color1, List color2){
            ...
            return(Math.abs(h1-h2)+Math.abs(s1-s2));
...
//            return Math.sqrt(Math.pow((r1 - r2), 2) + Math.pow((g1 - g2), 2) + Math.pow((b1 - b2), 2));
        ...
        public void setAlliancePipe(String alliance){
            if (alliance.equals("red")){
                ELEMENT_COLOR = Arrays.asList(0, 100, 0);
            }else{
                ELEMENT_COLOR = Arrays.asList(240, 100, 0);;
            }
        }
